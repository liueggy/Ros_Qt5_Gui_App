...
NavGoalWidget::NavGoalWidget(QWidget* parent) : QWidget(parent) {
  ...
  button_edit_name_->setStyleSheet(UiStyle::MainButtonStyleSheet());
  ...
  button_send_ = new QPushButton("前往点位");
  button_remove_ = new QPushButton("移除点位");
  button_cancel_ = new QPushButton("关闭");

  // 设置按钮对象名以便样式表识别
  button_remove_->setObjectName("deleteButton");
  button_cancel_->setObjectName("cancelButton");
  button_send_->setStyleSheet(UiStyle::MainButtonStyleSheet());
  button_remove_->setStyleSheet(UiStyle::DangerButtonStyleSheet());
  button_cancel_->setStyleSheet(UiStyle::SecondaryButtonStyleSheet());
